...
describe('useLottery', () => {
  ...
  it('parses names from a textarea-style string', () => {
    ...
    expect(lottery.participants.value).toEqual(['Amber', 'Jeroen', 'Cas'])
  })
  ...
  it('removes a participant by index', () => {
    ...
    expect(lottery.participants.value).toEqual(['Amber', 'Cas'])
  })
  ...
  it('removes the winner from the pool when auto-remove is enabled (default)', () => {
    ...
    expect(lottery.participants.value).toEqual(['Jeroen', 'Cas'])
  })
  ...
  it('prepends new winners so the log is newest-first', () => {
    ...
    lottery.recordWinner(1) // Jeroen

    expect(lottery.winners.value.map((w) => w.name)).toEqual(['Jeroen', 'Amber'])
